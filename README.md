...
TrustCode now includes three standalone entry points:

| Version | File | How to run | What it does |
|---|---|---|---|
...
| HTML version | `index.html` | Open it in a browser | Generates a local secret, authenticator import URI, and QR code in a polished browser interface. |
...
## Installation for Python versions
...
The HTML version generates:

- A local random secret
- An authenticator import URI
- A QR code for importing into authenticator apps
...
```text
totp authenticator 2fa qrcode security offline verification anti-scam python html developer-tools
```
...
| HTML网页版 | `index.html` | 用浏览器打开 | 在网页界面生成本地密钥、验证器导入链接和二维码。 |
